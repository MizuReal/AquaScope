...
export const useForumNotifications = ({
      ...
      const hydrated = await Promise.all(
        rawNotifications.map(async (item) => {
          ...
          const normalizedAvatar = normalizeAvatarUrl
            ? await Promise.resolve(normalizeAvatarUrl(actorProfile.avatar_url))
            : actorProfile.avatar_url || "";
          return {
            ...item,
            actorName: actorProfile.display_name || "Someone",
            actorAvatar: normalizedAvatar || actorProfile.avatar_url || "",
          };
        }),
      );
  ...
  const markAsRead = useCallback(async (notificationId) => {
    if (!sessionUserId || !notificationId || !supabase) return;
  ...
  useEffect(() => {
    if (!sessionUserId || !supabase) return undefined;
